...
> 이 문서는 **이 프로젝트를 처음 보는 사람**도 이해할 수 있도록 작성했습니다.
> "무엇을 하는 서비스인지 → 어떤 기술로 만들었는지 → 폴더가 어떻게 구성됐는지 → 어떻게 실행·배포하는지" 순서로 읽으면 됩니다.

---

## 1. 이 프로젝트가 무엇인가요? (비전공자용 설명)

병원의 의학 데이터는 여러 단계의 데이터베이스(DB)를 거치며 정리·통합·개방됩니다. 각 단계의 데이터가 **정해진 품질 기준**(예: 꼭 있어야 할 값이 비어있지 않은지, 형식이 맞는지)과 **통계 분포**(데이터가 정상 범위인지)를 만족하는지 검사해야 합니다.

**BIKO-DQM 대시보드**는 이 검사를 다루는 **관리자용 웹 화면**입니다. 구체적으로:
- 검증(검사)을 **실행**시키고,
- 검증 **결과를 표·차트로** 보여주고,
- 검증에 쓰이는 **지표·테이블 정의를 조회·관리**합니다.

이 저장소는 그중 **화면(프런트엔드)** 부분입니다. 실제 검사 로직과 데이터는 별도의 **백엔드 서버**와 **Airflow**가 담당하고, 이 웹은 백엔드에 요청을 보내 결과를 받아 보여줍니다.
...
| 라이브러리 | 버전 | 한 줄 설명(쉽게) |
|------------|------|------------------|
| `next` | 16.1.6 | 웹 프레임워크. 페이지 라우팅·서버 렌더링·API 프록시를 담당 |
| `react` / `react-dom` | 19.2.4 | 화면 UI를 그리는 핵심 라이브러리 |
| `typescript` | 5.7.3 | 타입이 있는 자바스크립트(오류를 미리 잡아줌) |
| `openapi-fetch` | ^0.17.0 | 백엔드 API를 **타입 안전**하게 호출하는 클라이언트 |
| `recharts` | 2.15.0 | 통계 결과의 **막대/분포 차트** 그리기 |
| `lucide-react` | ^0.564.0 | 아이콘 모음 (사이드바·버튼 아이콘 등) |
| `clsx` + `tailwind-merge` | ^2.1.1 / ^3.3.1 | className 조합 유틸(`cn()` 함수의 재료) |
| `class-variance-authority` | ^0.7.1 | 버튼/뱃지의 variant(스타일 변형) 정의 |
| `@radix-ui/react-*` | (아래) | 접근성 보장된 UI 기본 부품(드롭다운·다이얼로그 등) |
| `@vercel/analytics` | 1.6.1 | 방문 분석(선택적) |
...
> 참고: progress/스피너 등 단발성 UI는 라이브러리 대신 직접 구현해 의존성을 줄였습니다.
...
| `eslint` + `eslint-config-next` | ^9.39.4 / ^16.2.9 | 코드 린트(규칙 검사) |
| `openapi-typescript` | ^7.13.0 | 백엔드 OpenAPI → TS 타입 **생성기** (`pnpm api:gen`) |
| `@types/*` | — | 타입 정의 |

---

## 5. 프로젝트 구조 (폴더가 무슨 역할인지)

### 5.1 가장 중요한 규칙: "어디에 컴포넌트를 두나?"
- **여러 페이지에서 재사용** → 공통 폴더 **`components/`**
- **한 페이지에서만 사용** → 그 페이지 폴더의 **`_components/`**
- 화면 요소는 **카드는 카드, 표는 표, 모달은 모달**처럼 잘게 나눕니다. (한 페이지를 거대한 한 덩어리로 만들지 않음)
- 별도의 UI 래퍼 파일을 늘리지 않습니다. **공유되는 기본 UI만 `components/ui/`** 에 두고, 한 곳에서만 쓰는 UI는 그 컴포넌트 안에 직접 작성합니다.
...
lib/
  api/
    ...
    index.ts                          # 외부에 노출하는 진입점(클라이언트 + 도메인 타입)
