...
| 대상 | 주소 |
|------|------|
| 운영 웹 (이 프로젝트) | **http://175.106.96.71:8887** |
| 백엔드 QC 서버(API) | http://175.106.96.71:8081 (OpenAPI: `/v3/api-docs`) |
| Airflow API | (호스트 내부) 8080 |
| CloudBeaver(DB 콘솔) | http://175.106.96.71:8978 |

- 운영 서버는 **NCP**에 있으며, **SSH·웹 접속 모두 허용된(화이트리스트) 출발지 IP에서만** 가능합니다. IP가 바뀌면 재등록해야 합니다.
- 서버 SSH 접속 계정·배포 경로·저장소 자격증명 등 보안에 민감한 접근 정보는 저장소(README)에 두지 않습니다. (담당자에게 문의)
